...
RSpec.describe SidekiqUniqueJobs::Lock do
  ...
  let(:multilock)                      { described_class.new(multilock_item) }
  let(:multilock_resources)            { 2 }
  let(:multilock_expiration)           { nil }
  let(:multilock_use_local_time)       { false }
  let(:multilock_stale_client_timeout) { nil }
  let(:multilock_item) do
    {
      ...
      'unique_digest' => 'test_mutex_key',
      'args' => [1],
      'resources' => multilock_resources,
      'expiration' => multilock_expiration,
    ...
    describe 'lock with staleness checking' do
      ...
      context 'when redis_version is old' do
        ...
        it 'should restore resources of stale clients' do
          another_lock_item = lock_item.merge('resources' => 1, 'stale_client_timeout' => 1)
